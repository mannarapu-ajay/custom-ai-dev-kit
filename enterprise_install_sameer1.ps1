...
if ($EnterpriseSkillsMode -eq "git" -and $EnterpriseSkillsRepo) {
    ...
    $_mkInit = Join-Path $env:USERPROFILE ".ssh\id_ed25519_mccain"
    if (Test-Path $_mkInit) {
        try { $sshOut = (& ssh -o BatchMode=yes -o ConnectTimeout=5 -o "IdentityFile=$_mkInit" -o IdentitiesOnly=yes -T git@github.com 2>&1) | Out-String } catch { $sshOut = "" }
    } else {
